...
from .pmath import expmap0, _hyperbolic_softmax, expmap, mobius_add, dist, RiemannianGradient, project, mobius_matvec
...
class HyperbolicMLR(nn.Module):
    ...
    def forward(self, x, c=None):
        ...
        p_vals_poincare = expmap0(self.p_vals, c=c) # pmath.expmap0(self.p_vals, c=c)
        conformal_factor = 1 - c * p_vals_poincare.pow(2).sum(dim=1, keepdim=True)
        a_vals_poincare = self.a_vals * conformal_factor
        logits = _hyperbolic_softmax(x, a_vals_poincare, p_vals_poincare, c) # pmath._hyperbolic_softmax(x, a_vals_poincare, p_vals_poincare, c)
        return logits
...
class HypLinear(nn.Module):
    ...
    def forward(self, x, c=None):
        if c is None:
            c = self.c
        mv = mobius_matvec(self.weight, x, c=c) # pmath.mobius_matvec(self.weight, x, c=c)
...
class ConcatPoincareLayer(nn.Module):
    ...
    def forward(self, x1, x2, c=None):
        ...
        # pmath.mobius_add(self.l1(x1), self.l2(x2), c=c)
...
class HyperbolicDistanceLayer(nn.Module):
    ...
    def forward(self, x1, x2, c=None):
        ...
        # pmath.dist(x1, x2, c=c, keepdim=True)
...
class ToPoincare(nn.Module):
    ...
    def __init__(self, c, train_c=False, train_x=False, ball_dim=None, riemannian=True):
        ...
        self.riemannian = RiemannianGradient # pmath.RiemannianGradient
        self.riemannian.c = c
